...
- [Computers](#computers) :
  * [Form factor](#form-factor) : Timesharing capable mainframes, minicomputers (Datapoint 2200, Xerox Alto, Interdata 7/32 and 8/32), first commodity like computer (Apple II), first commercially available computer with a GUI (PERQ)
  * [OS](#os) : 
  * [Peripherals](#peripherals) : Dot matrix printer, DIN connector, Laser printers
  * [Storage](#storage) : Disk platter hard drive, hard drive swinging arm actuator, compact cassettes, 5+1⁄4-inch floppy disk drive 
  * [Uses](#uses) :
  
      *Servers* : FTP servers, Community memory (BBS precursor), Relational database management system (Oracle database)
      
      *Clients* : Timesharing / terminal emulation, interactive user terminals, first spreadsheet, first document preparation program
      
      *CGI* : Early CGI in movies, Westworld, Information International Inc - Motion Pictures Product Group, NYIT Computer Graphics Lab (CGL), Lucasfilm
- [Consumer Electronics](#consumer-electronics) :
  ...
  * [Video games](#video-games) : arcade video games (Pong), introduction of cartridge-based home consoles, first generation video games, Adventure games and Multi-User Dungeon(MUD), dedicated console market, second generation video games consoles
- [Standards and protocols](#standards-and-protocols) : X.25, TCP, FTP, TAR compression, EDI (X12), X.10, Modbus
- [Programming languages and frameworks](#programming-languages-and-frameworks) : Pascal (minicomputers, teaching language), Prolog (Natural language processing, rule-based logical queries), MESA (strong support for modular programming, software exceptions, thread synchronization, and incremental compilation), **ADA (ISO standard, military applications, critical real time applications), C (The Unix language, OS programming, embedded system applications)**, MATLAB (engineering matrix manipulation, plotting and UI)
