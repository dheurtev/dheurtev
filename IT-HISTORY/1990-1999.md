...
**In short:**
- Telecommunications : Dial-up internet access, Broadband internet (ADSL), Telecommunications Act of 1996, Local loop unbundling, Dot com bubble, Frame Relay, ATM, MPLS development, PBX to IP-PBX, VOIP
...
- Uses :
  * Servers : Web/internet (HTTP servers, HTML, URL, XML), Apache (web server), Microsoft Active Directory (Directory service), Asterisk (IP-PBX)
  * GUI : Microsoft Windows 3.0, 3.1, 95, 98
  * Web browsers : Netscape Navigator, Internet Explorer
  * CGI : Pixar (Toy Story)
- Consumer Electronics :
  * Hardware : High brightness blue led, First white LED sold, high-efficiency green light-emitting polymer-based device, idea of using quantum dots as light source, electronic ink
  * Distribution media : DVD
  * Gadgets : sophisticated calculators
  * TV / screens : HD resolution CRT, large plasma flat-panel TVs, LCD displays improvements (TN and IPS), HDTV with ATSC/DVB standards, XVGA computer monitors
  * Satellite : Medium power satellites with smaller dishes
  * Video games : Renaissance of arcade games, text-based multi-player virtual reality (Moo), fourth (Nintendo Super NES) to fifth generation video game consoles (Nintento 64, Sony Playstation)
- Standards and protocols : JPEG, MPEG-1, MPEG-2, WAV, MP3, PNG, SIP, SSH, VNC, Rsync, CORBA, ZIP compression tools and competitors (Winzip, RAR, ARJ, DEFLATE, 7-zip), CIFS, SOCKS, PPTP, Modems speeds (14.4k to 56k), HDTV (ATSC/DVB)
...
- **Pixar from hardware to 3D software to Toystory (1995)**
